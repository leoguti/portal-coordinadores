 ...
 *
 * REGLA PRINCIPAL:
 * - Días 1-5 del mes: Se pueden crear/editar/eliminar registros del mes actual y anterior
 * - Después del día 5: Solo se pueden crear/editar/eliminar registros del mes actual
 *
...
export function getFechaInicioModificable(): Date {
  ...
  if (diaActual > 5) {
    // Después del día 5: solo mes actual
    return new Date(hoy.getFullYear(), hoy.getMonth(), 1);
  } else {
    // Días 1-5: mes anterior y actual
...
export function getFechaCorteMesesCerrados(): Date {
  ...
  if (diaActual > 5) {
    // Después del día 5: mes actual está abierto, anteriores cerrados
    // Fecha corte: inicio del mes actual
    return new Date(hoy.getFullYear(), hoy.getMonth(), 1);
  } else {
    // Días 1-5: mes actual y anterior abiertos, anteriores cerrados
...
export function getMensajeErrorFecha(): string {
  ...
  if (diaActual > 5) {
    return "Solo se pueden modificar movimientos con fecha del mes actual después del día 5";
  } else {
    return "Solo se pueden modificar movimientos con fecha del mes actual y anterior durante los primeros 5 días del mes";
  }
}
 ...
 * REGLA: 5 días de gracia después del FIN DEL MES de la actividad
 * (Mismo comportamiento que kardex)
 *
 * @param fechaActividad - Fecha de la actividad (formato YYYY-MM-DD o Date object)
 * @returns true si puede ser modificada (dentro de 5 días), false si ya cerró
 */
...
export function getMensajeErrorActividad(fechaActividad: string | Date): string {
  ...
  return `Esta actividad es de ${mesNombre} y cerró el ${fechaCierreStr} (5 días después del fin de mes). Ya no se puede modificar ni eliminar.`;
}

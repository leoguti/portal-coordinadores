...
function CajaMenorPageInner() {
  ...
  // Coordinador "objetivo" para el PDF mensual:
  //   - Coordinador logueado: él mismo
  //   - Admin/Supervisor con filtro de coordinador: ese coordinador
  //   - Admin/Supervisor sin filtro: ninguno (son varios, no tiene sentido un PDF único)
  const coordinadorObjetivoId: string | undefined = canViewAll
    ? filtroCoordinador || undefined
    : session?.user?.coordinatorRecordId;

  const handleDescargarPDFMes = async (mes: string) => {
    if (!coordinadorObjetivoId) return;
    setGenerandoPDF(mes);
    try {
      const res = await fetch(`/api/caja-menor/pdf-mes?mes=${mes}&coordinador=${coordinadorObjetivoId}`);
      if (!res.ok) {
      ...
      a.href = url;
      a.download = `legalizacion-${mes}.pdf`;
  ...
  const handleDescargarFacturas = async (mes: string) => {
    if (!filtroCoordinador || !coordNombre) return;

    setDescargandoMes(mes);
    try {
      ...
      const response = await fetch(`/api/caja-menor/descargar-facturas?${params}`);
      ...
      if (response.ok) {
        ...
        const a = document.createElement("a");
        a.href = url;
        a.download = response.headers.get("Content-Disposition")?.split("filename=")[1]?.replace(/"/g, "") || `facturas_${mes}.zip`;
        document.body.appendChild(a);
        a.click();
        ...
        const data = await response.json();
        alert(`Error: ${data.error || "No se pudo generar el ZIP"}`);
      }
    } catch (err) {
      console.error("Error descargando facturas:", err);
                    ...
                    <div key={mes} className="bg-white rounded-lg shadow border border-gray-200 overflow-hidden">
                      ...
                      {coordinadorObjetivoId && (
                        <div className="flex justify-end px-4 py-2 bg-white border-b border-gray-200">
                          <button
                            onClick={() => handleDescargarPDFMes(mes)}
                            disabled={generandoPDF === mes}
                            className="flex items-center gap-2 px-3 py-1.5 bg-[#042726] hover:bg-[#032120] text-white text-xs font-medium rounded transition-colors disabled:opacity-50"
                          >
                            {generandoPDF === mes ? (
                              <>
                          ...
                          {canViewAll && filtroCoordinador && resumen.cantAprob > 0 && (
                            <div className="flex justify-end">
                              <button
                                ...
                                disabled={descargandoMes === mes}
                                className="flex items-center gap-2 px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium rounded transition-colors disabled:opacity-50"
                              >
                                {descargandoMes === mes ? (
                                  <>
                                    <svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24">
                                  ...
                                  <>
                                    ...
                                    Descargar Facturas ({resumen.cantAprob})
                                  </>
                                )}
                              </button>
                            </div>
